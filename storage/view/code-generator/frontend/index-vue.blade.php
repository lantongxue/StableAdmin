...
import { deleteByIds, page } from '~/{{ $codeGenerator->getPackageName() }}/api/{{ $codeGenerator->getName() }}.ts'
...
defineOptions({ name: '{{$codeGenerator->getPackageName()}}:{{Str::snake($codeGenerator->getName())}}' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('{{$codeGenerator->menu_i18n}}'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['{{$codeGenerator->getPackageName()}}:{{Str::snake($codeGenerator->getName())}}:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['{{$codeGenerator->getPackageName()}}:{{Str::snake($codeGenerator->getName())}}:delete']"
          type="danger"
          plain
