...
return useHttp().get('/admin/{{Str::snake($codeGenerator->getPackageName())}}/{{Str::snake($codeGenerator->getApiRouterName() ?: $codeGenerator->getName())}}/list', { params })
}

// {{$codeGenerator->menu_name}}新增
export function create(data: {{ $codeGenerator->getName() }}Vo): Promise<ResponseStruct<null>> {
  return useHttp().post('/admin/{{Str::snake($codeGenerator->getPackageName())}}/{{Str::snake($codeGenerator->getApiRouterName() ?: $codeGenerator->getName())}}', data)
}

// {{$codeGenerator->menu_name}}编辑
export function save(id: number, data: {{ $codeGenerator->getName() }}Vo): Promise<ResponseStruct<null>> {
    return useHttp().put(`/admin/{{Str::snake($codeGenerator->getPackageName())}}/{{Str::snake($codeGenerator->getApiRouterName() ?: $codeGenerator->getName())}}/${id}`, data)
}

// {{$codeGenerator->menu_name}}删除
export function deleteByIds(ids: number[]): Promise<ResponseStruct<null>> {
      return useHttp().delete('/admin/{{Str::snake($codeGenerator->getPackageName())}}/{{Str::snake($codeGenerator->getApiRouterName() ?: $codeGenerator->getName())}}', { data: ids })
}