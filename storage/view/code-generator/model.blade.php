@php use App\Model\CodeGenerator @endphp
@php /** @var CodeGenerator $codeGenerator */ @endphp
@php

    echo '<?php'.PHP_EOL;
    echo PHP_EOL;
    echo 'namespace App\\Model\\'.$codeGenerator->getPackageNameFirstUp() .  ($codeGenerator->getBackendChildPath() ? '\\'.str_replace('/', '\\', $codeGenerator->getBackendChildPath()).';' : ';') . PHP_EOL;
    echo PHP_EOL;
...
{
    protected ?string $table = '{{str_replace(config('databases.default.prefix'), '', $codeGenerator->table_name)}}';
